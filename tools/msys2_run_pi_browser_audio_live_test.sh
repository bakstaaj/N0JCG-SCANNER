...
Runs the V0.3H raw browser-audio quality test on the Pi and pulls the log
...
Options:
  ...
  --prebuffer-chunks N     Accepted for compatibility; ignored in raw V0.3H mode
  --declick-samples N      Accepted for compatibility; ignored in raw V0.3H mode
  --op25-verbosity N       Add OP25 -v N for D-Error/BER/frequency diagnostics
...
For diagnostic logging, try --op25-verbosity 10.
EOF_USAGE
}
...
cd "$1"
git pull
./tools/pi5_p25_run_with_log.sh --label browser_audio_live_test -- ./tools/pi5_p25_op25_browser_audio_live_test.sh --seconds "$2" --http-port "$3" --udp-port "$4" --prebuffer-chunks "$5" --declick-samples "$6" --op25-verbosity "$7" --yes
'

set +e
sshpass -p "$PI_PASSWORD" ssh -o StrictHostKeyChecking=accept-new "${PI_USER}@${PI_HOST}" \
  "bash -s -- '$PI_REPO' '$SECONDS_TO_RUN' '$HTTP_PORT' '$UDP_PORT' '$PREBUFFER_CHUNKS' '$DECLICK_SAMPLES' '$OP25_VERBOSITY'" <<< "$REMOTE_SCRIPT"
REMOTE_RC=$?
set -e
