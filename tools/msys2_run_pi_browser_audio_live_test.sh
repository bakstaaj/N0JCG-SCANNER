...
cd "$1"
git pull
./tools/pi5_p25_run_with_log.sh --label browser_audio_live_test -- ./tools/pi5_p25_op25_browser_audio_live_test.sh --seconds "$2" --http-port "$3" --udp-port "$4" --prebuffer-chunks "$5" --declick-samples "$6" --flag-drop-hold-ms "$7" --op25-verbosity "$8" --yes
'

set +e
sshpass -p "$PI_PASSWORD" ssh -o StrictHostKeyChecking=accept-new "${PI_USER}@${PI_HOST}" \
  "bash -s -- '$PI_REPO' '$SECONDS_TO_RUN' '$HTTP_PORT' '$UDP_PORT' '$PREBUFFER_CHUNKS' '$DECLICK_SAMPLES' '$FLAG_DROP_HOLD_MS' '$OP25_VERBOSITY'" <<< "$REMOTE_SCRIPT"
REMOTE_RC=$?
set -e
