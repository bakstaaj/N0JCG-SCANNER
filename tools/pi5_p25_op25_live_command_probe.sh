...
write_validated_marker() {
  ...
  local device_arg="$2"
  local log="$3"
  local existing_marker="runtime/settings/op25_validated_rx_command.env"
  local control_gain="LNA:$P25_CONTROL_GAIN_INT"
  local voice_gain="LNA:49"
  local control_demod="cqpsk"
  local voice_demod="fsk4"
  local voice_sample_rate="2400000"
  local voice_center_hz="852493750"
  if [[ -f "$existing_marker" ]]; then
    control_gain="$(grep '^P25_CONTROL_GAIN=' "$existing_marker" | tail -n 1 | cut -d= -f2- || true)"
    voice_gain="$(grep '^P25_VOICE_GAIN=' "$existing_marker" | tail -n 1 | cut -d= -f2- || true)"
    control_demod="$(grep '^P25_CONTROL_DEMOD_TYPE=' "$existing_marker" | tail -n 1 | cut -d= -f2- || true)"
    voice_demod="$(grep '^P25_VOICE_DEMOD_TYPE=' "$existing_marker" | tail -n 1 | cut -d= -f2- || true)"
    voice_sample_rate="$(grep '^P25_VOICE_SAMPLE_RATE=' "$existing_marker" | tail -n 1 | cut -d= -f2- || true)"
    voice_center_hz="$(grep '^P25_VOICE_CENTER_HZ=' "$existing_marker" | tail -n 1 | cut -d= -f2- || true)"
  fi
  control_gain="${control_gain:-LNA:$P25_CONTROL_GAIN_INT}"
  voice_gain="${voice_gain:-LNA:49}"
  control_demod="${control_demod:-cqpsk}"
  voice_demod="${voice_demod:-fsk4}"
  voice_sample_rate="${voice_sample_rate:-2400000}"
  voice_center_hz="${voice_center_hz:-852493750}"
...
P25_CONTROL_GAIN=$control_gain
P25_VOICE_GAIN=$voice_gain
P25_CONTROL_DEMOD_TYPE=$control_demod
