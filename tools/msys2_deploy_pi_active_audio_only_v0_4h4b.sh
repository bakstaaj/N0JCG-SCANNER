...
set -Eeuo pipefail
VERSION="v0_4h4b"
TITLE="Deploy V0.4H4B active-audio-only display using fixed Pi IP"
LOG_DIR="/c/Users/jim/Downloads/pi-p25-command-logs"
mkdir -p "$LOG_DIR" 2>/dev/null || true
STAMP="$(date -u +%Y%m%dT%H%M%SZ)"
LOG_FILE="$LOG_DIR/deploy_${VERSION}_active_audio_only_force_ip_${STAMP}.txt"
PASS_COUNT=0
WARN_COUNT=0
FAIL_COUNT=0
...
pass "deploy target forced to ${PI_USER}@${PI_HOST}:${PI_REPO}"
...
for _ in range(30):
    try:
        ...
        print("PROBE_OK", json.dumps({
            ...
            "start_enabled": (status.get("decoder_process") or {}).get("start_enabled"),
            "active_tgid": activity.get("active_tgid"),
            "label": activity.get("active_talkgroup_label"),
...
sys.exit(1)
PY
pass "probed /api/status and /api/activity via fixed IP"

cat > tools/msys2_probe_pi_active_audio_only_v0_4h4b.sh <<'PROBE'
#!/usr/bin/env bash
set -Eeuo pipefail
PI_HOST="192.168.254.63"
python3 - <<PY
...
PY
PROBE
chmod +x tools/msys2_probe_pi_active_audio_only_v0_4h4b.sh
pass "wrote fixed-IP activity probe helper"
