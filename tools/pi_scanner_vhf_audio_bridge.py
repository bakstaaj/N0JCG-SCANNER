#!/usr/bin/env python3
"""Isolated VHF UDP PCM to browser WAV bridge.

This service listens only to the VHF worker on UDP 23458 and exposes a separate
browser audio stream on HTTP 8073. It does not bind or alter the P25 ports.
"""
    ...
    def snapshot(self) -> dict[str, Any]:
        ...
        with self.lock:
            return {
                ...
                "mode": "isolated-vhf-browser-audio",
...
class AudioHandler(BaseHTTPRequestHandler):
    server_version = "PiScannerVhfAudio/0.1"

    def log_message(self, fmt: str, *args: Any) -> None:
        return

    @property
    def state(self) -> VhfAudioState:
...
def self_test() -> int:
    ...
    if not all(checks):
        ...
        print("FINAL: FAIL")
        return 1
    print("PASS: isolated VHF browser audio bridge self-test")
    print("FINAL: PASS")
    return 0
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        description="PI-SCANNER isolated VHF browser audio bridge"
    )
    ...
    state = VhfAudioState(max_queue_chunks=args.max_queue_chunks)
    ...
    print(
        "PI-SCANNER isolated VHF audio listening "
