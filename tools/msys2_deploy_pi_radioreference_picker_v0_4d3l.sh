...
LOG_FILE="${LOG_DIR}/deploy_v0_4d3l_radioreference_picker_${STAMP}.txt"
...
log "=== Deploy V0.4D3L forced RadioReference picker ==="
cd "$(dirname "$0")/.."
...
TMP_TAR="/tmp/pi_p25_rr_picker_v0_4d3l_${STAMP}.tar"
tar -cf "$TMP_TAR" src/pi_p25_scanner/backend.py src/pi_p25_scanner/radioreference_picker_forced_v0_4d3l.py
pass "built deploy tar"
"${SCP_CMD[@]}" "$TMP_TAR" "$REMOTE:/tmp/pi_p25_rr_picker_v0_4d3l.tar"
pass "copied deploy tar to Pi"
rm -f "$TMP_TAR"

"${SSH_CMD[@]}" "$REMOTE" "cd '$PI_REPO' && mkdir -p runtime/patch_backups/deploy_v0_4d3l_${STAMP} && cp -p src/pi_p25_scanner/backend.py runtime/patch_backups/deploy_v0_4d3l_${STAMP}/backend.py.bak 2>/dev/null || true && tar -xf /tmp/pi_p25_rr_picker_v0_4d3l.tar && python3 -m py_compile src/pi_p25_scanner/backend.py src/pi_p25_scanner/radioreference_picker_forced_v0_4d3l.py && sudo systemctl restart pi-p25-scanner.service && sleep 2"
...
if payload.get('picker_parser') != 'forced-explicit-soap-v0.4d3l':
    raise SystemExit('D3L parser did not take over: ' + str(payload.get('picker_parser')))
PY"
pass "verified D3L parser is active"
