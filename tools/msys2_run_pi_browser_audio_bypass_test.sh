...
usage() {
  cat <<'EOF_USAGE'
Usage:
  ./tools/msys2_run_pi_browser_audio_bypass_test.sh [options]

Runs the V0.3M raw browser-audio bypass test on the Pi and pulls the log
...
This bypass test intentionally disables project-side audio gates.
EOF_USAGE
}
...
cd "$1"
git pull
./tools/pi5_p25_run_with_log.sh --label browser_audio_bypass_test -- ./tools/pi5_p25_op25_browser_audio_bypass_test.sh --seconds "$2" --http-port "$3" --udp-port "$4" --op25-verbosity "$5" --yes
'

set +e
sshpass -p "$PI_PASSWORD" ssh -o StrictHostKeyChecking=accept-new "${PI_USER}@${PI_HOST}" \
  "bash -s -- '$PI_REPO' '$SECONDS_TO_RUN' '$HTTP_PORT' '$UDP_PORT' '$OP25_VERBOSITY'" <<< "$REMOTE_SCRIPT"
REMOTE_RC=$?
set -e

echo "Remote browser-audio bypass-test exit status: $REMOTE_RC"
...
if [[ -x ./tools/msys2_pull_latest_p25_log.sh ]]; then
  ./tools/msys2_pull_latest_p25_log.sh \
    ...
    --host "$PI_HOST" \
    --repo "$PI_REPO" \
    --label "browser_audio_bypass_test" \
