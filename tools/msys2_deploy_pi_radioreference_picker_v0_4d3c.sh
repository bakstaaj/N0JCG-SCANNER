...
LOG_FILE="$LOG_DIR/deploy_v0_4d3c_radioreference_picker_${STAMP}.txt"
...
log "=== Deploy V0.4D3C RadioReference picker state/county fix ==="
log "TARGET=$TARGET"
log "PI_REPO=$PI_REPO"

[[ -f src/pi_p25_scanner/radioreference_import.py && -f src/pi_p25_scanner/backend.py && -f web/app.js ]] || { fail "run from repo root"; finish || true; exit 1; }
...
TMP_TAR="/tmp/pi_p25_v0_4d3c_rr_picker_${STAMP}.tar"
tar -cf "$TMP_TAR" src/pi_p25_scanner/radioreference_import.py src/pi_p25_scanner/backend.py web/app.js
pass "created deploy tar"
"${SSH[@]}" "$TARGET" "test -d '$PI_REPO'"
pass "remote repo exists"
"${SCP[@]}" "$TMP_TAR" "$TARGET:/tmp/pi_p25_v0_4d3c_rr_picker.tar"
pass "uploaded deploy tar to $PI_HOST"
"${SSH[@]}" "$TARGET" "cd '$PI_REPO' && mkdir -p runtime/patch_backups/deploy_v0_4d3c_${STAMP} && cp src/pi_p25_scanner/radioreference_import.py src/pi_p25_scanner/backend.py web/app.js runtime/patch_backups/deploy_v0_4d3c_${STAMP}/ && tar -xf /tmp/pi_p25_v0_4d3c_rr_picker.tar && python3 -m py_compile src/pi_p25_scanner/radioreference_import.py src/pi_p25_scanner/backend.py && if command -v node >/dev/null 2>&1; then node --check web/app.js; fi"
