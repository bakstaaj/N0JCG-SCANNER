...
STATUS_PATH="${STATUS_PATH:-/tmp/pi-scanner-vhf-smoke-status.json}"
BRIDGE_HTTP_PORT="${BRIDGE_HTTP_PORT:-18073}"
BRIDGE_UDP_PORT="${BRIDGE_UDP_PORT:-24458}"
BRIDGE_LOG="${BRIDGE_LOG:-/tmp/pi-scanner-vhf-bridge-smoke.log}"
BRIDGE_PID=""
...
pass "VHF worker self-test passed"

python3 tools/pi_scanner_vhf_audio_bridge.py --self-test
pass "VHF audio bridge self-test passed"

rm -f "$BRIDGE_LOG"
python3 tools/pi_scanner_vhf_audio_bridge.py \
  --host 127.0.0.1 \
...
print("VHF_BRIDGE_NETWORK_SMOKE=PASS")
PY
pass "VHF bridge UDP-to-HTTP status path passed"
...
timeout 20 python3 -m pi_p25_scanner.analog_vhf_worker \
  ...
  --smoke-seconds 8 \
  --no-forward
pass "VHF RTL hardware PCM smoke command completed"
...
if failed:
    raise SystemExit(
        f"VHF hardware smoke status failed {failed}: {status}"
    )
