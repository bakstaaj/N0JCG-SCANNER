...
let browserAudioLastEvent = 'Start Scanner + Browser Audio will start playback.';
...
function renderBrowserAudioState() {
  ...
  const state = audio ? `${audio.paused ? 'paused' : 'playing'} / readyState ${audio.readyState}` : 'audio element missing';
  const badgeText = browserAudioPlaying ? 'Playing' : 'Ready';
  const badgeKind = browserAudioPlaying ? 'badge-ok' : 'badge-warn';
  setBadge('browserAudioBadge', badgeText, badgeKind);
  setText('browserAudioContextState', state);
...
async function playBrowserAudioStream() {
  const audio = field('browserAudioPlayer');
  if (!audio) {
  ...
  const streamUrl = rawAudioStreamUrl();
  ...
  try {
    ...
    browserAudioLastEvent = `Browser audio stream playing from ${streamUrl}`;
    renderBrowserAudioState();
    return true;
  } catch (error) {
    browserAudioPlaying = false;
    browserAudioLastEvent = `Audio autoplay was blocked or the stream is not ready. Press Play on the audio control. ${error.message}`;
    renderBrowserAudioState();
    return false;
  }
}

function pauseBrowserAudioStream() {
  const audio = field('browserAudioPlayer');
  if (audio) {
    audio.pause();
...
function renderDashboard(status) {
  ...
  if (running) {
    ...
    setText('dashboardSummary', `Scanner is ready. Click Start Scanner + Browser Audio once to start OP25 and play audio.`);
  } else {
...
async function startScannerWithBrowserAudio() {
  ...
  try {
    ...
    browserAudioLastEvent = 'Starting scanner and attaching browser audio...';
    renderBrowserAudioState();
    const status = await fetchJson('/api/scanner/start', { method: 'POST' });
    renderStatus(status);
    await playBrowserAudioStream();
  } catch (error) {
    browserAudioPlaying = false;
    browserAudioLastEvent = `Start/audio error: ${error.message}`;
...
field('browserAudioPlayer')?.addEventListener('play', () => {
  browserAudioPlaying = true;
  browserAudioLastEvent = `Browser audio stream playing from ${rawAudioStreamUrl()}`;
  renderBrowserAudioState();
});
field('browserAudioPlayer')?.addEventListener('pause', () => {
  browserAudioPlaying = false;
  browserAudioLastEvent = 'Browser audio paused.';
  renderBrowserAudioState();
});
field('browserAudioPlayer')?.addEventListener('error', () => {
  browserAudioPlaying = false;
  browserAudioLastEvent = 'Browser audio stream error. Confirm the raw audio service is running on port 8072.';
  renderBrowserAudioState();
});
