...
def discover_radioreference_systems(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    state_id = _rr_picker_find_id(
        country_info,
        state,
        ("stid", "stateId", "state_id", "id"),
        ("stateName", "name", "state", "stateCode", "code", "abbr", "shortName"),
    ) if country_info is not None else None

    state_info = None
    if state_id is not None:
        state_info = _rr_picker_call(
            client,
            "getStateInfo",
            [
                {"__kwargs__": {"stid": state_id, "authInfo": auth}},
                {"__kwargs__": {"stateId": state_id, "authInfo": auth}},
                (state_id, auth),
                (auth, state_id),
    ...
    # Some WSDL versions expose direct trunked-system discovery methods. Try them
    # without failing if they are absent.
    for method_name in ("getTrsByState", "getTrsSystemsByState", "getTrsListByState"):
        direct = _rr_picker_call(
            client,
            method_name,
            [
                {"__kwargs__": {"stid": state_id, "authInfo": auth}},
                {"__kwargs__": {"stateId": state_id, "authInfo": auth}},
                {"__kwargs__": {"state": state, "authInfo": auth}},
                {"__kwargs__": {"stateCode": state, "authInfo": auth}},
                (state_id, auth),
                (state, auth),
                (auth, state_id),
                (auth, state),
    ...
    county_id = _rr_picker_find_id(
        state_info,
        county,
        ("ctid", "countyId", "county_id", "id"),
        ("countyName", "name", "county", "descr", "description"),
    ) if state_info is not None else None

    county_info = None
    if county_id is not None:
        county_info = _rr_picker_call(
            client,
            "getCountyInfo",
            [
                {"__kwargs__": {"ctid": county_id, "authInfo": auth}},
                {"__kwargs__": {"countyId": county_id, "authInfo": auth}},
                (county_id, auth),
                (auth, county_id),
    ...
    for method_name in ("getTrsByCounty", "getTrsSystemsByCounty", "getTrsListByCounty"):
        direct = _rr_picker_call(
            client,
            method_name,
            [
    ...
    for source, value in sources:
        for system in _rr_picker_extract_systems(value, source=source, searched=searched):
            existing = systems_by_id.get(int(system["system_id"]))
            if existing is None or int(system.get("rank", 50)) < int(existing.get("rank", 50)):
                systems_by_id[int(system["system_id"])] = system

    systems = sorted(systems_by_id.values(), key=lambda s: (int(s.get("rank", 50)), str(s.get("name", "")).lower(), int(s.get("system_id", 0))))
    return {
        "ok": True,
        "searched": searched,
...
def discover_radioreference_sites(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    sources: list[tuple[str, Any]] = []
    for method_name in ("getTrsSites", "getTrsSiteList"):
        value = _rr_picker_call(
            client,
            method_name,
            [
    ...
    if details is not None:
        sources.append(("getTrsDetails", details))
    ...
    for source, value in sources:
        for item in _iter_dicts(value):
            ...
            site_id = _rr_picker_site_id(item)
            name = _rr_picker_site_name(item)
            has_site_key = any(str(k).lower() in {"siteid", "site_id", "sitenumber", "siteno", "sitename", "site_name"} for k in item.keys())
    ...
    sites.sort(key=lambda s: (s.get("site_id") is None, int(s.get("site_id") or 999999), str(s.get("name", "")).lower()))
    if not sites:
        all_freqs: list[int] = []
