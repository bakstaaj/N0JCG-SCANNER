#!/usr/bin/env python3
"""Isolated UHF analog scanner worker for PI-SCANNER.

This phase intentionally handles only the UHF receiver role (analog_70cm) bound
to RTL-SDR serial 00000144. Active 8 kHz signed 16-bit mono PCM frames are sent
to UDP port 23459. P25 services and the VHF receiver are not modified.
"""
...
DEFAULT_STATUS_PATH = PROJECT_ROOT / "runtime" / "status" / "analog_70cm.json"

ROLE = "analog_70cm"
EXPECTED_SERIAL = "00000144"
EXPECTED_UDP_PORT = 23459
...
class UhfWorkerError(RuntimeError):
    """Raised when UHF configuration or runtime validation fails."""
...
def normalize_mode(value: Any) -> str:
    ...
    if mode not in aliases:
        raise UhfWorkerError(f"unsupported analog mode: {value!r}")
    return aliases[mode]
...
def load_uhf_config(
    ...
    if not source_path.exists():
        raise UhfWorkerError(
            f"analog configuration missing: {config_path} and {template_path}"
        )
    try:
        payload = json.loads(source_path.read_text(encoding="utf-8"))
    except json.JSONDecodeError as exc:
        raise UhfWorkerError(f"invalid analog configuration JSON: {exc}") from exc
    if not isinstance(payload, dict):
        raise UhfWorkerError("analog configuration must be a JSON object")

    workers = payload.get("workers")
    if not isinstance(workers, dict):
        raise UhfWorkerError("analog configuration must contain workers")
    raw_worker = workers.get(ROLE)
    if not isinstance(raw_worker, dict):
        raise UhfWorkerError(f"analog configuration is missing {ROLE}")
    ...
    if audio_rate_hz != EXPECTED_AUDIO_RATE_HZ:
        raise UhfWorkerError(f"{ROLE} must use {EXPECTED_AUDIO_RATE_HZ} Hz audio")
    if frame_bytes != EXPECTED_FRAME_BYTES:
        raise UhfWorkerError(f"{ROLE} must use {EXPECTED_FRAME_BYTES}-byte frames")
    if udp_port != EXPECTED_UDP_PORT:
        raise UhfWorkerError(f"{ROLE} must use UDP port {EXPECTED_UDP_PORT}")
    ...
    for index, raw in enumerate(raw_worker.get("channels") or []):
        if not isinstance(raw, dict):
            raise UhfWorkerError(f"{ROLE} channel {index + 1} must be an object")
    ...
    if not channels:
        raise UhfWorkerError(f"{ROLE} has no enabled channels")
...
class UhfScannerWorker:
    ...
    def status_payload(self, state: str) -> dict[str, Any]:
        ...
        return {
            ...
            "browser_audio_url": "http://DEVICE-IP:8074/audio.wav",
    ...
    def run_channel(self, channel: dict[str, Any]) -> None:
        ...
        try:
            ...
            raise UhfWorkerError("rtl_fm is not installed") from exc
...
def self_test() -> int:
    ...
    if not all(checks):
        ...
        print("FINAL: FAIL")
        return 1
    print("PASS: isolated UHF worker self-test")
    print("FINAL: PASS")
    return 0
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        description="PI-SCANNER isolated UHF analog scanner worker"
    )
    ...
    config = load_uhf_config(Path(args.config), Path(args.template))
    if args.print_config:
        print(json.dumps(config, indent=2))
        return 0

    worker = UhfScannerWorker(
        config,
