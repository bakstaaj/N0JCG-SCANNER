"""Forced RadioReference picker routes for PI-P25-SCANNER V0.4D3L.
...
PARSER_MARKER = "forced-explicit-soap-v0.4d3l"
US_COID = 2
...
def find_systems(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    try:
        country_info = client.service.getCountryInfo(coid=US_COID, authInfo=auth)
        state_id, state_match, state_candidates = _find_state(country_info, state)
    except Exception as exc:
        call_errors.append({"method": "getCountryInfo", "error": f"{type(exc).__name__}: {exc}"})
